...
import { getCoursesByCateDesignActions } from '../../../../redux/actions/coursesByCategoryActions';

export default function TabCoursesDesign(props) {
    const {arrCourseByCate} = useSelector((state) => state.CoursesByCategoryReducer);
    console.log(arrCourseByCate)

    const dispatch = useDispatch();

    useEffect (() =>{
        dispatch(getCoursesByCateDesignActions)
    },[dispatch])

 const renderCoursesDesign = () => {
    return arrCourseByCate.slice(0,8).map((course, index) => {
      return (
        <div
          key={index}
